...
import com.jojikubota.android.rentme.Model.SavedSearch;
...
public class SavedSearchesFragment extends Fragment {
    ...
    private class SavedHolder extends RecyclerView.ViewHolder
        ...
        public SavedHolder(View itemView)  {
            ...
            mKeyword = (TextView) itemView.findViewById(R.id.saved_keyword);
            mCity = (TextView) itemView.findViewById(R.id.saved_city);
            mType=(TextView) itemView.findViewById(R.id.saved_type);
            mRent = (TextView) itemView.findViewById(R.id.saved_rent);
...
//            Intent intent = UpdatePostDetailActivity.newIntent(getActivity(), mPosting.getPostId());
            Intent intent = SearchResultDetailActivity.newIntent(getActivity(), mSavedSearch.getKeyword());

            startActivity(intent);
        }

        public void bindPosting(SavedSearch savedSearch) {
            mSavedSearch = savedSearch;

            mKeyword.setText(mSavedSearch.getKeyword());
            mCity.setText(mSavedSearch.getCity());
            mType.setText(mSavedSearch.getType());
            mRent.setText(mSavedSearch.getRent());
        }
    }

    // Adapter
    private class SavedSearchAdapter extends RecyclerView.Adapter<SavedHolder> {

        private List<SavedSearch> mSavedSearch;

        public SavedSearchAdapter(List<SavedSearch> savedSearches) {
            mSavedSearch = savedSearches;
        }
        ...
        public void onBindViewHolder(SavedHolder holder, int position) {
            SavedSearch savedSearch = mSavedSearch.get(position);
            holder.bindPosting(savedSearch);
        }

        @Override
        public int getItemCount() {
            return mSavedSearch.size();
        }
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mPostingRecyclerView = (RecyclerView) view
                .findViewById(R.id.saved_search_recycle_view);
        ...
        mAdapter = new SavedSearchAdapter(savedSearches);
        ...
        String query=activity.getSearchQuery();

        getPostingFromDb(query);
...
//        List<Posting> postings = postingList.getPostings();
        List<SavedSearch> savedSearches = mSavedSearch;
    ...
    private void getPostingFromDb(final String query) {
    ...
    private void setNavigationDrawer(View view) {
        ...
        mNavigationView = (NavigationView) view.findViewById(R.id.navigation_view_saved_search);
        ...
        mDrawerLayout = (DrawerLayout) view.findViewById(R.id.drawer_layout_saved_search);
        mDrawerToggle  =
